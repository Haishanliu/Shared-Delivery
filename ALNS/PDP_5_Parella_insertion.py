...
class Model():
    def __init__(self):
        ...
        self.alpha = 0.3
        self.beta = 0.7
        self.Max_order_number = 20
        self.power_p = 5
        self.relate_f1 = 9
        self.relate_f2 = 3
        self.removal_frac = 0.2
...
class Sol(): ## given the orders and generated routes, what is the objective value? routes: array of route object
    ...
    def copy(self):
        copy_sol = Sol(self.routes)
        copy_sol.obj = self.obj
        copy_sol.total_distance =self.total_distance
        copy_sol.delay = self.delay
        return copy_sol
    ...
    with open(task_file,'r') as f:
        ...
        for row in task_reader:
            ...
            if int(row['id']) %2 ==0: # pick_up
                ...
                model.task_dict[task.id] = task
                model.task_id_list.append(task.id)
        model.total_task_number = len(model.task_id_list)
...
def sol_objective(sol,model): # calculate solution objective, total distance and total delay
    alpha = model.alpha
    beta = model.beta
    distance_total = 0
    delay_total = 0
    ...
    sol.obj = alpha*distance_total+beta*delay_total
...
def initial_distance_time_estimation(task_sequence,model): # only for one route
    '''
    ...
    if len(task_sequence)==0: # empty route
        ...
        start_time = model.task_dict[task_sequence[0]].EPT # first task should be pick up
        ...
        for i in range(len(task_sequence) - 1):
            ...
            if task_sequence[i+1]%2 == 0: # pick_up point
                ...
                if current_time > model.task_dict[task_sequence[i+1]].ETA:
                    ...
                    one_delay = current_time - model.task_dict[task_sequence[i+1]].ETA
                    if one_delay>20:
                        distance = float('inf')
                        delay = float('inf')
                        duration= float('inf')
        ...
        route_obj = alpha*distance + beta*delay
        return distance,delay,duration,route_obj

## feasibility control #控制前后，控制由同一辆车送一个订单，控制订单数，控制司机的schedule，控制

def distance_time_estimation(task_sequence,model): # only for one route
    '''
    :param task_sequence: given the task sequence, the driver starts working 10 min before the first EPT. And the driver is in the restaurant already
    '''
    alpha = model.alpha
    ...
    if len(task_sequence)==0: # empty route
        ...
        for i in range(len(task_sequence) - 1):
            ...
            if task_sequence[i+1]%2 == 0: # pick_up point
                ...
                if current_time > model.task_dict[task_sequence[i+1]].ETA:
                    # model.task_dict[task_sequence[i + 1]].delay = current_time - model.task_dict[task_sequence[i+1]].ETA
                       delay += current_time - model.task_dict[task_sequence[i+1]].ETA
                else:
                   pass
                    # model.task_dict[task_sequence[i + 1]].delay = 0
        duration = current_time - start_time
...
def feasibility_control(task_sequence,model): ## 给入一个 route，判断是否可行,
    max_order_number = model.Max_order_number
...
def orders_to_remove(model):
    return random.randint(8,int(model.total_task_number*model.removal_frac))# total task number to remove

# removal operators#

def random_removal(current,model):
    pool = []
    print('random_removal')
    destroyed = current.copy()
    for i in range(orders_to_remove(model)):
        key_pool = []
...
def worst_removal(current,model): # remove the top_N worst orders based on the objective value
    ...
    n_insert_cost = n_insert_cost[n_insert_cost[:, 2].argsort(kind = 'heapsort')] # sort by insertion cost,increasing
    q = orders_to_remove(model)
    while q>0:
        y = random.uniform(0, 1) # randomly choose a number from [0,1)
        index = int(np.shape(n_insert_cost)[0]*(y**model.power_p))
...
def relateness(model,r1,r2): # two task id. f1,f2 weight for dis and time_diff # from Ropke paper
    ...
    relate = model.relate_f1*(r1_dis+ r2_dis)+model.relate_f2*(r1_time_diff+r2_time_diff)
    return relate
...
def shaw_removal(current,model):
    ...
    if item%2 ==0: #pick_up point
        ...
        destroyed.routes[key].remove(item-1)
        destroyed.routes[key].remove(item)
    while len(pool)<orders_to_remove(model):
        ...
        n_relate = n_relate[n_relate[:, 2].argsort(kind = 'heapsort')]  # sort by relatedness 越小，相似性越高
...
def random_repair(current,pool,model):
    ...
    return current
def initial_greedy_insertion(current,pool,model):
...
def initial_findGreedyInsert(current,pool,r_cost,model): # find best orders to be inserted in best position
            ...
            for k in range(len(route)+1): # +1, cause it can insert at the end of the route.
                for j in range(k+1,len(route)+2): # +1, cause it can insert at the end of the route.
                    ...
                    if feasibility_control(temp_route,model): # check feasibility firstt, then calculate objective
                        cost = initial_distance_time_estimation(temp_route,model)[3] # after insertion
...
# def nearest_driver(driver_list,task_id,model): #driver_list: id of available drivers
#     distance = []
#     for driver_id in driver_list:
#         distance.append(nx.shortest_path_length(G,source=driver.loc[driver_id,'id'],target= model.task_dict[task_id].location_id,weight='length'))
#     # print(distance)
...
#         chosen_driver_id = nearest_driver(driver_list,task_pool[0],model)
...
#         print('chosen_driver_id',chosen_driver_id)
#         insert_list = []
#         no_improvement = 0
...
#                 print(chosen_driver_id,solution.routes[chosen_driver_id])
#                 break
...
def doDestroy(destroy_id,current,model):
    if destroy_id == 0:
        destroyed,pool = random_removal(current,model)
    elif destroy_id ==1:
...
def driver_provider_matching(solution,task_pool):
    driver_restaurant = pd.read_csv(
        r"C:\Users\Haishan Liu\PycharmProjects\pythonProject\PDPTW\testcode\scenario_1\restaurant_driver_pair_routing_result.csv")
    # initial first order to each driver
    k = driver.shape[0]  # k number of driver
    chosen_driver = []
    i = 0
    for iter in range(driver.shape[0]):
        index = driver_restaurant.iloc[0:k]['travel_distance'].idxmin()
        driver_location = driver_restaurant.iloc[index]['to']
        driver_id = driver[driver['id'] == driver_location]['unique_id'].values[0]
        solution.routes[driver_id].append(task_pool[i])
        solution.routes[driver_id].append(task_pool[i + 1])
        # print(solution.routes)
        driver_restaurant = driver_restaurant.iloc[k:]  # 先把前面已经insert的order 删掉
        driver_restaurant = driver_restaurant.loc[driver_restaurant['to'] != driver_location].reset_index(drop=True)
        # print(driver_restaurant)
        i = i + 2
        k = k - 1
def run(routing_result_file,task_file,driver_file,r1,r2,r3,rho,phi,alpha,beta,max_order_number,relate_f1,relate_f2,power_p,removal_frac,epochs,pu): #还有其他参数 shaw removal f1,f2,p (power of the random number)
    '''
    ...
    model.alpha =alpha
    model.beta = beta
    model.Max_order_number = max_order_number
    model.relate_f1 = relate_f1
    model.relate_f2 = relate_f2
    model.power_p = power_p
    model.removal_frac = removal_frac
    ...
    start_1 = time.time()
    driver_provider_matching(sol,model.task_id_list)
    print(sol.routes)
    pool = model.task_id_list[20:] # insert
    sol = initial_greedy_insertion(sol,pool,model)
    ...
    for ep in range(epochs):
        ...
        for k in range(pu):
            ...
            destroyed,pool= doDestroy(destroy_id,current_sol,model)
            new_sol = doRepair(repair_id,destroyed,pool,model)
            if new_sol.obj < current_sol.obj:
...
if __name__=='__main__':
    ...
    run(routing_result_file, task_file, driver_file,r1=33,r2=15,r3=9,rho=0.8,phi=0.999, alpha=3.33, beta = 0.3, max_order_number =20, relate_f1 =9,
        relate_f2 = 3, power_p = 5, removal_frac = 0.4, epochs= 20, pu= 50)
